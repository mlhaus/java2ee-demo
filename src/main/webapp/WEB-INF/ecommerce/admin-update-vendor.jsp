...
<head>
    ...
    <title>Update Vendors Admin</title>
        ...
        <c:otherwise>
            ...
            <form class="row g-3" method="POST" action="update-vendor?id=${id}">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control <c:choose><c:when test="${vendorIdError == true}">is-invalid</c:when><c:when test="${vendorIdError == false}">is-valid</c:when><c:otherwise></c:otherwise></c:choose>" id="vend_id" name="vend_id" value="${vendor.vend_id}">
